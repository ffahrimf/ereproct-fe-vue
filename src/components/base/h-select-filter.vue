<template>
  <div>
    <div class="flex items-center justify-between max-[768px]:block mb-2">
      <div>
        <label class="text-xs font-medium text-gray-400" :for="id">{{
          label
        }}</label>
      </div>
      <button
        @click="emit('update:modelValue', '')"
        class="text-blue-600 font-medium active:scale-95"
      >
        Clear
      </button>
    </div>
    <button
      :class="`py-1.5 px-2.5 flex items-center justify-between rounded-lg outline-none cursor-pointer ${
    ...
    <transition>
      ...
      >
        ...
        <div class="pb-3" v-if="loading">
          <ProgressLinear />
          <p
            class="text-xs text-center text-slate-500 mt-1"
            v-if="!items?.length"
          >
            Loading...
          </p>
        ...
        >
          <div
            v-for="item in items"
            class="mb-2 hover:bg-gray-100 p-2 rounded text-xs cursor-pointer last:mb-0"
        ...
        >
          <h-icon
            ...
            size="18"
          ></h-icon>
          <p class="text-xs text-center text-slate-700 ml-1">
