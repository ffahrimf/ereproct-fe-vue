<template>
  <table-layout icon="user" title="Student List" subtitle="Manage your Student">
...
import { StudentIF } from "./student.interface";
...
const getStudent = (): void => {
  loading.value = true;
  console.log(query.name);
  const q = useQuery(query);
  api.get(`user${q}`).then((res) => {
    console.log(res);
    const raw: StudentIF[] | [] = res.data.items;
    data.value = raw;
