  ...
  <h-dialog :dialog="dialog">
    ...
    >
      <div class="flex w-full px-2 items-center justify-between">
        <p class="font-medium text-slate-700 text-sm">Change Proctor to</p>
...
const form = reactive<ChangeProctor>({
  id: "",
...
const changeProctor = (): void => {
  ...
  api
    .post(req)
    .then(() => {
      useToast("Berhasil mengubah Proctor", "success");
      emit("close");
      emit("refetch");
      loading.value = false;
    })
