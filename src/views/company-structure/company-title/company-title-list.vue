    ...
    <div class="grid grid-cols-2 mb-3 px-5">
      <div>
        <div class="flex items-center gap-1">
          <h-icon name="identification" class="text-primary"></h-icon>
          <h1 class="text-lg">Company Title</h1>
        </div>
      </div>
...
import { CompanyTitleIF } from "./company-title.interface";
import DialogDelete from "../../../components/dialog-delete.vue";
import CreateCompanyTitle from "./create-company-title.vue";
...
const getCompanyTitle = (): void => {
  loading.value = true;
  const q = useQuery(query);
  api.get(`company-title${q}`).then((res) => {
    const raw: CompanyTitleIF[] | [] = res.data.items;
    data.value = raw;
...
const onUpdate = (item: CompanyTitleIF) => {
  pocket.value = {
    id: item.id,
...
const onDelete = (item: CompanyTitleIF) => {
  pocket.value = {
    path: `company-title/delete/${item.id}`
  };
  dialog.delete = true;
};
