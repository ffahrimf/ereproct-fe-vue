    ...
    >
      <div class="py-2 px-6 border-b flex items-center justify-between">
        <p class="font-medium text-slate-700 text-sm">
          {{ form.id ? "Update Company Level" : "Create Company Level" }}
        </p>
      ...
      <div class="py-3 px-6 flex-1">
        ...
        <div class="mb-3">
          <h-input
            label="Company Level"
            placeholder="Ketik Tingkat Perusahaan..."
            v-model="form.name"
          ></h-input>
        ...
        <div class="mb-3">
          <h-input
            label="Description"
            placeholder="Ketik Deskripsi Tingkat Perusahaan"
...
const form = reactive<CreateCompanyLevel>({
  id: "",
  company_id: null,
  name: "",
  description: ""
...
const postData = (): void => {
  ...
  let req = {
    path: `company-level/${form.id ? "update" : "create"}`,
    body: body
  };
