  ...
  <h-dialog :dialog="dialog">
    ...
    >
      <div class="py-2 px-6 border-b flex items-center justify-between">
        <p class="font-medium text-slate-700 text-sm">Create Mentor</p>
      ...
      <div class="py-3 px-6 flex-1 styled-scroll">
        ...
        <div class="mb-3">
          <h-input
            label="Email"
            placeholder="Ketik Email Mentor"
            v-model="form.email"
            :disabled="!!form.id"
          ></h-input>
...
const form = reactive<CreateMentor>({
  id: "",
  name: "",
  username: "",
  password: "",
...
const postData = (): void => {
  loading.value = true;
  const mentorRole: number = 2;
  form["role_id"] = mentorRole;
