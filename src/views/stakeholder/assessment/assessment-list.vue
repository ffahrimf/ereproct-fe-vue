<template>
  <table-layout title="Assessment List">
    <template #header-searchbar>
      <h-search
        v-model="query.name"
        placeholder="Search"
        @search="getAssessment"
        @keyup="loading = true"
...
import { AssessmentIF } from "./assessment.interface";
import DialogDelete from "../../../components/dialog-delete.vue";
import CreateAssessment from "./create-assessment.vue";
...
const getAssessment = (): void => {
  ...
  api.get(`assessment${q}`).then((res) => {
    const raw: AssessmentIF[] | [] = res.data.items;
    data.value = raw;
...
const changeStatus = (item: AssessmentIF) => {
  loading.value = true;
  const body = {
    ...item,
    status: !item.status
...
const onDelete = (item: AssessmentIF) => {
  pocket.value = {
    path: `assessment/delete/${item.id}`
  };

  dialog.delete = true;
};

const onChangePassword = (item: AssessmentIF) => {
  pocket.value = {
    id: item.user.id
  };
