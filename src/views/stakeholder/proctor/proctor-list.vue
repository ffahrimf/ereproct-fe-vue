<template>
  <table-layout title="Proctor List">
...
import { ProctorIF } from "./proctor.interface";
import DialogDelete from "../../../components/dialog-delete.vue";
import CreateProctor from "./create-proctor.vue";
...
const getProctor = (): void => {
  ...
  api.get(`proctor${q}`).then((res) => {
    const raw: ProctorIF[] | [] = res.data.items;
    data.value = raw;
...
const changeStatus = (item: ProctorIF) => {
  loading.value = true;
  const body = {
    ...item,
    status: !item.status
...
const onDelete = (item: ProctorIF) => {
  pocket.value = {
    path: `proctor/delete/${item.id}`
  };

  dialog.delete = true;
};

const onChangePassword = (item: ProctorIF) => {
  pocket.value = {
    id: item.user.id
  };
