  ...
  <h-dialog :dialog="dialog">
    ...
    >
      ...
      <p class="text-center text-sm mb-7 mt-2 leading-6 px-3">
        Apakah anda yakin laporan proctoring
        {{ props.pocket.value.event.code }} sudah diisi dengan benar?
      </p>
...
const submitReport = (): void => {
  ...
  const req = {
    ...
    body: props.pocket.value
  };

  api
    .post(req)
    .then(() => {
      useToast("Laporan berhasil di submit", "success");
      emit("close");
      emit("refetch");
      loading.value = false;
      router.push("/report");
